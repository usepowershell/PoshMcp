...
- 2026-06-02: Release finalization for `v0.16.4` should be gated by commit identity parity across main HEAD, green workflows, and remote tag peel target; all three matched `9a4cc931ac5720f0fb305fccebe6da9dad8c6b66` in the final check.
- 2026-06-03: Minor version v0.17.0 released for `/mcp` route alias (clawpilot/Microsoft Scout compatibility). Version bump, CHANGELOG, release notes, and tag all committed and pushed. GitHub release creation via `gh` requires browser auth scope refresh in non-interactive environments; release notes are in repo and tag is remote-accessible.