...
## Archived 2026-05-05 (history summarization, lines 201-734 of pre-summarization file)
...
## Learnings
- 2026-05-29T11:46:53.2558064-05:00: v0.16.1 release-gate diagnosis: GitHub CI for b37f485 ultimately completed successfully, so the release tag was not blocked by the suspected Integration hang. Local probes still showed `ToolDescriptionParityTests` can be lifecycle-sensitive when run as a full class after repeated `HelpParityFixtureSession` startup/teardown, while the individual theory groups passed; treat that as an existing integration/OOP harness concern, not fallout from the session-aware runspace fix.
- 2026-05-29T11:46:53.2558064-05:00: Hermes' `SessionAwarePowerShellRunspace` regression coverage is a good unit-level patch gate when it proves explicit `Mcp-Session-Id` isolation, same-session reuse, and the no-header/no-context fallback to the shared `default` runspace. For release readiness on this surface, run the focused class first, then the full `Category=Unit` tier because singleton runspace behavior is shared HTTP infrastructure.
- 2026-05-22T05:50:13.171-05:00: `SettingsResolver` unit coverage fits best as a dedicated pure-C# test file that calls the internal helpers directly, reuses a local `EnvironmentVariableScope`, and leans on `[Theory]` + `[InlineData]` for CLI/env precedence matrices. Issue #294 also exposed that `ParseLogLevel` needed explicit `Critical` and `None` mappings to match the full `Microsoft.Extensions.Logging.LogLevel` surface.
- 2026-05-28: Prompt templating QA pass for `McpPromptHandler` validation + rendering changes. Added integration coverage for required-argument edge cases that were previously untested: explicit `null` value and whitespace-only value now both assert `-32602 InvalidParams` and mention `serviceName`. Added metadata stability regression guard to ensure `prompts/list` output is unchanged before/after `prompts/get` execution (protects against accidental prompt metadata mutation during template rendering work).
- 2026-05-16T17:02:54.700-05:00: Issue #272 test shape mirrors spec 010's `DoctorDescriptionSourceTests`: add a small unit seam around tracker → doctor entry serialization, then a separate integration parity test for real InProcess vs OutOfProcess behavior. New files: `PoshMcp.Tests/Unit/Diagnostics/DoctorToolImportSourceTests.cs` and `PoshMcp.Tests/Integration/ToolImportParityTests.cs`.
- 2026-05-16T17:02:54.700-05:00: For import-source parity, a multi-module config is the key acceptance gate because the old heuristic reports non-`CommandNames` tools as `unknown` when `Modules.Count > 1`. The parity test uses `CommandNames = [Get-Date]`, `Modules = [Microsoft.PowerShell.Management, Microsoft.PowerShell.Archive]`, and `IncludePatterns = [Out-String]` so one fixture exercises commandName, module, and pattern attribution in the same report.
