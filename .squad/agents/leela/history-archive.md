...
- **20260528T000000Z**: ✓ Prompt templating documentation refresh — updated prompt docs in `README.md` and `docs/articles/resources-and-prompts.md` to describe source-driven rendering (`file` via `Path`, `command` via `Command`), prompt metadata expectations (required `Name`, recommended `Description`, argument metadata), runtime behavior for `prompts/list` and `prompts/get`, required-argument enforcement at runtime, and dual placeholder syntax support (`{{argName}}` preferred, `{argName}` backward-compatible). Kept examples concise and implementation-faithful.
- **20260529T114653-0500**: ✓ v0.16.1 release note prep — documented Hermes' Streamable HTTP runspace fix as a patch release: sessionless HTTP traffic now reuses the stable `default` runspace, while real `Mcp-Session-Id` requests keep per-session isolation. For operational fixes, name the platform symptom first (health/readiness probes causing unbounded runspaces) and keep upgrade notes as drop-in.
- **20260529T114653-0500**: ✓ v0.16.2 release note prep — documented the configured command resource executor fix as a runtime parity patch: simple command resources use the active `ICommandExecutor` so module-imported commands are visible to `resources/read`, while script/pipeline resources keep the runspace fallback. For resource execution fixes, name both the affected source shape and the preserved fallback path.
