...
- Skip-enumeration count-guard (Fix 1+2, commit 6c2f29f): capture `table.Count` for var/func/alias tables at the end of `CreateWorkerAsync` (after alias snapshot, before `TryTransitionTo(Warm)`). In `ResetCore`, compare live count to baseline — skip `Remove*FromTable` when equal. Cost savings ~0.5–1.0ms per clean warm call. Safety: PS `Constant` automatic variables cannot be removed, so net count is a reliable user-additions counter. Net-zero (add+remove same request) correctly skips.
- When validating count-guard tests, use `GetVariable -ErrorAction SilentlyContinue` rather than assigning to a bool directly — the latter forces a pipeline that can shadow `$null` return as `$false`.
- Never rely on working-tree state across branch switches in a shared worktree directory. Commit before any risk of a branch checkout by a sibling agent. Use `git stash` as a safety net for long builds/tests.