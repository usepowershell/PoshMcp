...
.DESCRIPTION
    ...
    Use -GenerateDockerfile with 'build' or 'build-custom' to write a documented
    copy of the Dockerfile to disk instead of running docker build.
.PARAMETER Command
...
.PARAMETER GenerateDockerfile
    When set with 'build' or 'build-custom', writes the Dockerfile content to disk
    with a header comment instead of running docker build. Useful for inspecting or
    archiving the exact Dockerfile that would be used for a given build invocation.
.PARAMETER OutputPath
    Path to write the generated Dockerfile when -GenerateDockerfile is used.
    Defaults to './Dockerfile.generated' for base builds and
    './Dockerfile.<Template>.generated' for custom builds.
    Ignored when -GenerateDockerfile is not set.
.EXAMPLE
    .\docker.ps1 build
...
.EXAMPLE
    .\docker.ps1 build -GenerateDockerfile
    Write the base Dockerfile to ./Dockerfile.generated with a header comment
.EXAMPLE
    .\docker.ps1 build -GenerateDockerfile -OutputPath ./my-base.dockerfile
    Write the base Dockerfile to a custom path
.EXAMPLE
    .\docker.ps1 build-custom -Template azure -GenerateDockerfile
    Write the azure template Dockerfile to ./Dockerfile.azure.generated
.EXAMPLE
    .\docker.ps1 build-custom -Template user -Tag dev -GenerateDockerfile -OutputPath ./out/user.dockerfile
    Generate the user template Dockerfile at a custom output path
.EXAMPLE
...
switch ($Command) {
    { $_ -in 'build', 'build-base' } {
        ...
        if ($GenerateDockerfile) {
            $resolvedOutput = $OutputPath ? $OutputPath : './Dockerfile.generated'
            $buildCmd = "docker build -t $imageTag ."
            $timestamp = Get-Date -Format 'o'
            $sourceFile = './Dockerfile'

            if (-not (Test-Path $sourceFile)) {
                Write-Host "[X] Source Dockerfile not found: $sourceFile" -ForegroundColor Red
                exit 1
            }

            $sourceContent = Get-Content $sourceFile -Raw
            $header = @"
# Generated by PoshMcp docker.ps1
# Equivalent build command: $buildCmd
# Generated: $timestamp
#
# To build this image, run:
#   docker build -f $resolvedOutput -t $imageTag .

"@

            Set-Content -Path $resolvedOutput -Value ($header + $sourceContent) -NoNewline
            Write-Host ''
            Write-Host "[OK] Dockerfile written to: $resolvedOutput" -ForegroundColor Green
            Write-Host "     Equivalent build command: $buildCmd" -ForegroundColor Cyan
            Write-Host ''
        }
        else {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Host ''
                Write-Host "[OK] Base Docker image built successfully: $imageTag" -ForegroundColor Green
                Write-Host ''
    ...
    'build-custom' {
        ...
        if ($GenerateDockerfile) {
            $resolvedOutput = $OutputPath ? $OutputPath : "./Dockerfile.${Template}.generated"
            $buildCmd = "docker build -f $dockerfilePath -t $customImageTag ."
            $timestamp = Get-Date -Format 'o'
            $sourceContent = Get-Content $dockerfilePath -Raw
...
#
# To build this image, run:
#   docker build -f $resolvedOutput -t $customImageTag .

"@

            if ($Template -eq 'azure') {
                $envNote = @"
# Azure template env vars: AZURE_CLIENT_ID, AZURE_TENANT_ID, AZURE_SUBSCRIPTION_ID

"@
                $header = $header + $envNote
            }
            ...
            if (-not $baseImageCheck) {
                ...
                if ($LASTEXITCODE -ne 0) {
                    Write-Host '[X] Failed to build base image' -ForegroundColor Red
                    exit 1
                }
            }
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Host ''
                Write-Host "[OK] Custom Docker image built successfully: $customImageTag" -ForegroundColor Green
                Write-Host ''
                ...
                if ($Template -eq 'azure') {
                    Write-Host ''
                    Write-Host 'Azure Template Notes:' -ForegroundColor Cyan
                ...
                Write-Host ''
                Write-Host '[X] Custom Docker build failed' -ForegroundColor Red
                exit 1
            }
        }
    }
    ...
    'run' {
        ...
        switch ($Mode) {
            { $_ -in 'web', 'http' } {
                ...
                if ($LASTEXITCODE -eq 0) {
                    Write-Host '[OK] PoshMcp Web Server is running at http://localhost:8080' -ForegroundColor Green
            ...
            { $_ -in 'stdio', 'server' } {
                ...
                if ($LASTEXITCODE -eq 0) {
                    Write-Host '[OK] PoshMcp stdio Server is running' -ForegroundColor Green
                    Write-Host "     Use '.\docker.ps1 logs' to view logs" -ForegroundColor Cyan
                    Write-Host "     Use '.\docker.ps1 stop' to stop the server" -ForegroundColor Cyan
            ...
            default {
                Write-Host "[X] Unknown mode: $Mode" -ForegroundColor Red
    ...
    'stop' {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host '[OK] PoshMcp Servers stopped' -ForegroundColor Green
        }
    }
